...
**Structured JSON:** Chat Completions accepts OpenAI-compatible `response_format` values of `json_object` and `json_schema`. Hermes adds the requested JSON constraint to the server-side agent prompt and rejects malformed or unsupported `response_format` values with `400`; clients must still validate returned JSON against their own schema before persistence.
