...
| `LINE_ALLOWED_USERS` | Comma-separated user IDs allowed to DM the bot and send in an allowed group or room (`U`-prefixed). |
| `LINE_ALLOWED_GROUPS` | Comma-separated group IDs the bot will respond in (`C`-prefixed); each sender must also be in `LINE_ALLOWED_USERS`. |
| `LINE_ALLOWED_ROOMS` | Comma-separated room IDs the bot will respond in (`R`-prefixed); each sender must also be in `LINE_ALLOWED_USERS`. |
