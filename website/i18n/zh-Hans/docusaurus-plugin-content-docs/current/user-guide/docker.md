...
内置 Web dashboard 在明确启用时，会在同一容器内作为受 s6-rc 监管的服务与 gateway 并行运行。它默认关闭；设置 `HERMES_DASHBOARD=1`（或 `true` / `yes`）可在容器回环地址（`127.0.0.1`）启动它。
...
| 环境变量 | 描述 | 默认值 |
|---------------------|-------------|---------|
| `HERMES_DASHBOARD` | 设为 `1`、`true` 或 `yes` 可启用受监管的 dashboard 服务；任何其他值都会关闭它 | 已关闭 |
