...
# Fork attribution map. Upstream keeps ~1900 frozen legacy entries here plus a
# one-file-per-email directory under contributors/emails/; this fork carries
# only its own maintainers, which is what its release tooling needs to resolve.
#
# Upstream's directory mechanism (loaded below) is retained: it stays the
# conflict-free place to add a NEW contributor, and it wins over this dict on
# duplicates. Entries live here rather than as files only because the fork's
# own two mappings predate the directory and upstream's contributor-map tests
# require a non-empty dict.
LEGACY_AUTHOR_MAP = {
    "ad6885971@gmail.com": "LoopAllan",
